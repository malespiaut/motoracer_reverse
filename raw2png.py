...
def main():
    ...
    with open(sys.argv[1], "rb") as f:
        ...
        if str(f.read(6), "ascii") == "mhwanh":
            ...
            # Reading header
            image_width: int = int.from_bytes(f.read(2), byteorder="big")
            image_height: int = int.from_bytes(f.read(2), byteorder="big")
            logging.info(f"Image size: {image_width}x{image_height}")

            palette_colors: int = int.from_bytes(f.read(2), byteorder="big")
            ...
            if palette_colors != 0:
                ...
                with open(filename_gen(sys.argv[1], "pal"), "wb") as output_palette:
                    ...
                    output_image: Image.Image = Image.frombytes(
                        ...
                        f.read(image_width * image_height),
                    )
                ...
                Image.fromarray(
                    np.frombuffer(f.read(), dtype=np.uint8).reshape(
